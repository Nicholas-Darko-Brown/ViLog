...
app.post("/", (req, res) => {
    ...
    if(name && company && tel && email && position && host && timestamp && password){db.query(insertVisitorQuery, [name, company, tel, email, host, position, `checked in on ${timeIn.toLocaleDateString()} at ${timeIn.toLocaleTimeString()}`, DAY, MONTH, YEAR, password], (err, result) =>{
        if(err){
            console.log(err);
        }else{
...
app.post("/visitorLogin", (req, res)=>{
    ...
    const SelectHostEmailQuery = `${select} ${fullNameCol1}, ${emailCol1} ${from} ${employeesTable} ${where} ${idCol1} = ?`;
               ...
               if( password === result[0].Password){
                   ...
                   db.query(insert, [result[0].Full_name, result[0].Company, result[0].Phone_Number, email, host, position, `checked in on ${timeIn.toLocaleDateString()} at ${timeIn.toLocaleTimeString()}`, DAY, MONTH, YEAR, hash], (err, rslt)=>{
                       if(err){
                           ...
                           db.query(SelectHostEmailQuery, [host], (err, result2) =>{
                            if(err){
                                console.log(err);
                            }else{
                                ...
                                const mailOptions = {
                                    ...
                                    to: result2[0].Email,
                                    subject: "Message from vilogtext@gmail.com: ViLog checkIn",
                                    text: `Hello ${result2[0].Full_Name}. Your visitor ${result[0].Full_name} is waiting for you.`
                                };
